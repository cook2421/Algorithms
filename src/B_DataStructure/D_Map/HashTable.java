...
/*
* 분리연결법(Separate Chaining) (개방주소법(Open Addressing)은 값이 많아질수록 성능 이슈가 있음)
* */
public class HashTable {
    ...
    // key값으로 Hash Table에 저장될 index를 반환한다.
    ...
    // 저장할 데이터로 key를 추출한다.
    // 실제 Hash에는 특별한 알고리즘이 적용되어 hashCode를 얻는다.
    public int getKey(int data){
        return data;
    }


    // data를 Hash Table에 저장한다.
    public int add(int data){
        // data의 key(hashCode)와 저장할 index를 얻는다.
        int key = getKey(data);
        ...
        // Hash Table의 저장할 index가 비어있다면, 저장하고 끝낸다.
        ...
        while(true){
            if(cur == null){                 // Hash Table에 저장할 index가 비어있으면
                ...
                pre = cur;
                cur = cur.next;
            } else {                         // 해당 index의 커서 노드의 값이 key보다 크다.(여기에 저장)
                // 커서 노드가 Hash Table의 첫번째 노드이면
                if(cur == hTable[hashValue]){
                    //삽입 노드를 첫번째 노드로 삽입하고 다음 노드를 커서 노드로 지정한다.
                    entry.next = cur;
                    hTable[hashValue] = entry;
                    return 0;
                } else {    // 첫번째 노드가 아니면
    ...
    public int get(int data){
        ...
        while(true){
            if(cur == null){                // index가 비어있다면 -1 반환
                ...
                return hashValue;
            } else if(cur.value > key){     // 커서의 값이 키보다 크면 -1 반환 (리스트는 작은 값에서 큰 값으로 정렬되어 있다)
                return -1;
    ...
    private Entry getEntry(int data){
        ...
        // Hash Table의 index의 첫번째 노드와 두번째 노드
        Entry pre = hTable[hashValue];
        Entry cur = pre.next;

        while(true){
            if(cur == null){                // 커서 노드가 null이면 null 반환
                return null;
    ...
    public int remove(int data){
        ...
        // data가 있는 노드의 전 노드를 가져오고 null이면 -1 반환
        if((pre=getEntry(data)) == null){
            return -1;
        }

        // 전 노드가 data의 다음 노드를 가리키게 한다.
